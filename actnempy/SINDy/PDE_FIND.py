...
def get_relative_error(lhs,rhs):
    """
    This is a function to compute the relative error betwen the predicted
    rhs and the ground truth lhs.
    It does this by computing the ratio of the minor axis and the major 
    axis of the approximate ellipse that would be formed by plotting the 
    scatter plot of (lhs,rhs)
    """

    R = np.array([lhs,rhs]).T
...
def TrainSTRidge(R, Ut, lam, d_tol, maxit = 25, STR_iters = 10, l0_penalty = None, normalize = 2, split = 0.8, print_best_tol = False):
    ...
    # Mod by Chaitanya - Implement binary search here?
    for iter in range(maxit):
        
        # print(f'Iter # {iter}: tol: {tol:.2e}')
        # Get a set of coefficients and error
        w = STRidge(R,Ut,lam,STR_iters,tol,normalize = normalize)
        err_lsq = np.linalg.norm(TestY - TestR.dot(w), 2)
        err_l0  = l0_penalty*np.count_nonzero(w)
        err = err_lsq + err_l0
    ...
    # print(f"err_best = {err_lsq} in Least Sq + {err_l0} in L0 Norm")

    lhs = np.squeeze(np.real(Ut))
    rhs = np.squeeze(np.real(R.dot(w_best)))
...
def TrainSTRidgeSelect(R, Ut, lam, d_tol, maxit = 25, STR_iters = 10, 
                       term_penalties=None, l0_penalty = None, normalize = 2,
    ...
    if term_penalties is None:
        term_penalties = np.ones((D,1))
    else:
        if (len(term_penalties) != D):
            raise ValueError
        else:
            term_penalties[term_penalties!=0] = 1.0
    
    # Get the standard least squares estimator
    w = np.zeros((D,1))
    w_best = np.linalg.lstsq(TrainR, TrainY, rcond=None)[0]
    # err_best = np.linalg.norm(TestY - TestR.dot(w_best), 2) + l0_penalty*np.count_nonzero(w_best)
    err_best = np.linalg.norm(TestY - TestR.dot(w_best), 2) + l0_penalty*np.count_nonzero(w_best*term_penalties[:,None])
    tol_best = 0
    ...
    for iter in range(maxit):
        ...
        # Get a set of coefficients and error
        # w = STRidge(R,Ut,lam,STR_iters,tol,normalize = normalize)
        w = STRidgeSelect(R,Ut,lam,STR_iters,tol,normalize = normalize,term_penalties=term_penalties)
        # err = np.linalg.norm(TestY - TestR.dot(w), 2) + l0_penalty*np.count_nonzero(w)
        err_lsq = np.linalg.norm(TestY - TestR.dot(w), 2)
        err_l0  = l0_penalty*np.count_nonzero(w*term_penalties[:,None])
        # err = np.linalg.norm(TestY - TestR.dot(w), 2) + l0_penalty*np.count_nonzero(w*term_penalties)
        err = err_lsq + err_l0
        # Has the accuracy improved?
        if err <= err_best:
            err_best = err
...
def TrainSTRidgeBinary(R, Ut, lam, 
                       tol_min, tol_max, maxit = 25, STR_iters = 10,
                       l0_penalty = None, normalize = 2, split = 0.8, 
                       print_best_tol = False):
    """
    This function trains a predictor using STRidge.

    It runs over different values of tolerance and trains predictors on
    a training set, then evaluates them using a loss function on a
    holdout set.

    Please note published article has typo.  Loss function used here for
    model selection evaluates fidelity using 2-norm, not squared 2-norm.
    """

    # Split data into 80% training and 20% test, then search for the
    # best tolerance.
    np.random.seed(0) # for consistancy
    n,_ = R.shape
    train = np.random.choice(n, int(n*split), replace = False)
    ...
    avg = lambda a, b : np.sqrt( a * b ) # Using Logarithmic averaging
    if l0_penalty == None: l0_penalty = 0.001*np.linalg.cond(R)
    print(f"l0_penalty: {l0_penalty}")
    # Get the standard least squares estimator
    w = np.zeros((D,1))
    w_best = np.linalg.lstsq(TrainR, TrainY)[0]
    err_prev = 1e6 # Something large
    err_best = np.linalg.norm(TestY - TestR.dot(w_best), 2) + l0_penalty*np.count_nonzero(w_best)
    tol_best = 0
    ...
    for iter in range(maxit):
        ...
        tol = avg(tol_min,tol_max)

        relative_tol = np.abs( np.log(tol/tol_prev) )
        relative_err = np.abs( (err_prev-err_best)/err_prev )
        print(f"relative_tol: {relative_tol}")
        print(f"relative_err: {relative_err}")
        if( (relative_tol < 1e-3)  & (relative_err < 1e-6) ):
        # if( (relative_err < 1e-6) ):
           print("Binary Search Converged!")
           break
        print(f'Iter # {iter}: tol: {tol:.2e}')
        tol_prev = tol
...
def HTRidge(X0, y, lam, normalize = 2, print_results = False):
    """
    Heirarchically Thresholded Ridge Regression algorithm for finding (hopefully) sparse 
    approximation to X^{-1}y.  The idea is that this may do better with correlated observables.

    This assumes y is only one column
    
    Instead of cutting off values smaller than an arbitrary tolerance, 
    we cut off the value with the smallest coefficient, one at a time.
    ...
    for j in range(1,d):
        ...
        # Figure out which items to cut out
        smallinds = np.where(abs(w) <= np.min(abs(w[np.nonzero(w)])))[0]
...
def HLSq(X0, y, normalize = 2, print_results = False):
    """
    Heirarchical Least Squares algorithm for finding (hopefully) sparse 
    approximation to X^{-1}y.  The idea is that this may do better with correlated observables.
...
def STRidgeSelect(X0, y, lam, maxit, tol, 
                  normalize = 2, term_penalties=None, print_results = False):
    """
    Sequential Threshold Ridge Regression algorithm for finding (hopefully) sparse 
    approximation to X^{-1}y.  The idea is that this may do better with correlated observables.
    ...
    if term_penalties is None:
        ...
        if (len(term_penalties) != d):
            ...
            term_penalties[term_penalties!=0] = 1.0
            GTG = np.diag(term_penalties) # G^{-1}G = G
    # if lam != 0: w = np.linalg.lstsq(X.T.dot(X) + lam*np.eye(d),X.T.dot(y))[0]
    if lam != 0: w = np.linalg.lstsq(X.T.dot(X) + lam*GTG,X.T.dot(y), rcond=None)[0]
    else:
        w = np.linalg.lstsq(X, y, rcond=None)[0]
    num_relevant = d
    # biginds = np.where( abs(w) > tol)[0] 
    biginds = np.union1d(np.where(term_penalties == 0)[0],np.where(abs(w) >= tol)[0])
    ...
    for j in range(maxit):
        ...
        # Figure out which items to cut out
        # smallinds = np.where( abs(w) < tol)[0]
        smallinds = np.intersect1d(np.where(term_penalties != 0)[0],np.where(abs(w) < tol)[0])
        new_biginds = [i for i in range(d) if i not in smallinds]
        ...
        if lam != 0:
            if term_penalties is None:
                w[biginds] = np.linalg.lstsq(X[:, biginds].T.dot(
                    X[:, biginds]) + lam*np.eye(len(biginds)), X[:, biginds].T.dot(y), rcond=None)[0]
            else:
                term_penalties_reduced = term_penalties[biginds]
                GTG_reduced = np.diag(term_penalties_reduced)
                w[biginds] = np.linalg.lstsq(X[:, biginds].T.dot(
                    X[:, biginds]) + lam*GTG_reduced, X[:, biginds].T.dot(y), rcond=None)[0]
        else:
            w[biginds] = np.linalg.lstsq(X[:, biginds], y, rcond=None)[0]
    ...
    if normalize != 0:
        w_reg = np.multiply(Mreg,w)
        err_lsq = np.linalg.norm(y - X0.dot(w_reg), 2)
        return w_reg
    else: 
        err_lsq = np.linalg.norm(y - X0.dot(w), 2)
        return w
