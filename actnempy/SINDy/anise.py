...
class Anise(ActNem):
    ...
    def __init__(self, data_dir, visual_check=False, run=False):
        ...
        if os.path.isfile(self.processed_data_file):
            ...
            vals = [self.u_all.shape, self.v_all.shape, self.Qxx_all.shape, self.Qxy_all.shape]
    ...
    def generate_libraries_int(self, overdamped=False):
        """
        ...
        base_Stokes = build_base_expr(funcsStokes, coords, constraints_Stokes)
        ...
        if overdamped:
            ...
            base_Od = build_base_expr(funcsOd, coords, constraints_Od)
        ...
        lib_Q = build_constrained_library_array(funcsQ, base_Q, db_Q, coords, constraints_Q)

        lib_Stokes = build_constrained_library_array(
            funcsStokes, base_Stokes, db_Stokes, coords, constraints_Stokes
        )

        # Add time derivative term to the Stokes library
        dw_dt = get_term_val(lib_lhs, "(∂ω/∂t)")
        # Append ω to the Stokes library
        dw_dt_struct = np.array([("(∂ω/∂t)", dw_dt.flatten())], dtype=lib_Stokes.dtype)
        lib_Stokes = np.append(lib_Stokes, dw_dt_struct)

        if overdamped:
            lib_overdamped = build_constrained_library_array(
                funcsOd, base_Od, db_overdamped, coords, constraints_Od
            )
        ...
        (
            points,
            views,
        ) = get_random_sample(self.u_all.shape, num_windows, window_size, diff_order, sample)
        ...
        for key, view in views.items():
            ...
            lib_Q_p = build_constrained_library_array(funcsQ, base_Q, db_Q_p, coords, constraints_Q)

            lib_Stokes_p = build_constrained_library_array(
                funcsStokes, base_Stokes, db_Stokes_p, coords, constraints_Stokes
            )

            # Add time derivative term to the Stokes library
            dw_dt_p = get_term_val(lib_lhs_p, "(∂ω/∂t)")
            # Append ω to the Stokes library
            dw_dt_struct_p = np.array([("(∂ω/∂t)", dw_dt_p.flatten())], dtype=lib_Stokes_p.dtype)
            lib_Stokes_p = np.append(lib_Stokes_p, dw_dt_struct_p)

            if overdamped:
                lib_overdamped_p = build_constrained_library_array(
                    funcsOd, base_Od, db_overdamped_p, coords, constraints_Od
                )
    ...
    def weak_form_flow_libs(self, num_windows, window_size, sample=1, p=6, r=1):
        """
        ...
        (
            _,
            views,
        ) = get_random_sample(self.size, num_windows, window_size, diff_order, sample)
        ...
        lib_lhs = np.array(
            [("∇²u", grad_sq_U_int)], dtype=[("name", "U50"), ("val", "float", grad_sq_U_int.shape)]
        )

        return (lib_lhs, lib_St)

    def q_tensor(self, num_windows, window_size, sample=1, p=3, r=1):
        ...
        for key, view in tqdm(views.items()):
            ...
            U_dot_grad_Q_int[key] = -2 * np.mean(
                Qxx * (u * dx_Rxx + v * dy_Rxx) + Qxy * (u * dx_Rxy + v * dy_Rxy)
            )
        ...
        lib_DQ = np.array(
            [("E", shear_int), ("Q", Q_int), ("TrQ² \u00d7 Q", trQ2_Q_int), ("∇²Q", grad_sq_Q_int)],
            dtype=[("name", "U50"), ("val", "float", U_dot_grad_Q_int.shape)],
        )

        lib_lhs = np.array(
            [("(∂Q/∂t)", dt_Q_int), ("(DQ/Dt)", Dt_Q_int)],
            dtype=[("name", "U50"), ("val", "float", U_dot_grad_Q_int.shape)],
        )
    ...
    def weak_form(self, num_windows, window_size, sample=1, p=6, r=1):

        (lib_lhs, lib_St) = self.weak_form_flow_libs(num_windows, window_size, sample, p, r)
    ...
    def weak_form_Q(self, num_windows, window_size, sample=1, p=3, r=1):

        (lib_lhs, lib_Q, lib_DQ) = self.q_tensor(num_windows, window_size, sample, p, r)
