...
class Benchmark(Anise):
    ...
    def stokes_int(self):
        ...
        (lib_lhs, lib_Q, lib_NS, lib_Stokes, lib_overdamped) = self.generate_libraries_int()

        print("Computing the PDE for Stokes flow...")
        pde_St = PDE(lib_Stokes, lib_lhs, "∇²ω", self.metadata)
        print("Done! Stored under pde_St.\n")

        ida1 = np.argwhere(pde_St.rhs["name"] == "(∂²Qxy/∂x²)").flatten()[0]
        ida2 = np.argwhere(pde_St.rhs["name"] == "(∂²Qxx/∂x∂y)").flatten()[0]
        ida3 = np.argwhere(pde_St.rhs["name"] == "(∂²Qxy/∂y²)").flatten()[0]
    ...
    def stokes_weak(self):
        ...
        (lib_lhs, lib_NS, lib_St) = self.weak_form_flow_libs(1, 500, (45, 45, 101))
    ...
    def weak_form_benchmark_window_size(self, noise_strength=1e-1):
        ...
        np.savez(
            f"{self.data_dir}/weak_form_benchmark.npz",
            r2s=r2s,
            alphas=alphas,
            frictions=frictions,
            Ws=np.array([WXs, WYs, WTs]),
        )

        fig, ax = plt.subplots()
        ax.plot(np.arange(1, num_sizes + 1), r2s_mean, color="m")
        ax.fill_between(
            np.arange(1, num_sizes + 1),
            r2s_mean - r2s_std,
            r2s_mean + r2s_std,
            alpha=0.3,
            color="m",
        ...
        ax.fill_between(
            np.arange(1, num_sizes + 1), a_mean - a_std, a_mean + a_std, alpha=0.3, color="g"
        )
    ...
    def run(self):
        ...
        for i, noise_strength in enumerate(noise_levels):
            ...
            print(f"Noise strength: {noise_strength}, Int: {alphas_int[i]}, Weak: {alphas_weak[i]}")
